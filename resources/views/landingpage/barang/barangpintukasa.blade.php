...
@section('content')
    ...
    <div class="container-md mx-auto overflow-hidden mt-2 mb-5 animated fadeInUp">
        ...
        <div class="row g-4 justify-content-center">
            @foreach ($pintu as $key => $pintu)
                <div class="col-lg-4 col-md-6 wow fadeInUp" data-wow-delay="0.1s">
                    <div class="service-item bg-light overflow-hidden h-100">
                        <img class="img-fluid" src="{{ asset('Images/uploads/pintukasa') }}/{{ $pintu->foto }}" alt="" />
                        <div class="service-text position-relative text-center h-100 p-4">
                            <h5 class="mb-3">{{ $pintu->judul }}</h5>
                            <p>
                                {{ $pintu->deskripsi }}
                            </p>
                            <a class="small" href="/detailbarangpintukasanyamuk/{{ $pintu->id }}">READ MORE<i class="fa fa-arrow-right ms-3"></i></a>
                        </div>
                    </div>
                </div>
            @endforeach
